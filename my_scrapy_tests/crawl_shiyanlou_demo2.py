...
写完之后, 运行spider:
scrapy runspider my_scrapy_tests/crawl_shiyanlou_demo2.py -o data.json  # 输出格式为 json, 看到的文件是unicode编码的
scrapy runspider my_scrapy_tests/crawl_shiyanlou_demo2.py -o output.xml  # 输出格式为 .xml, 看到的汉子比较清楚.
...
class ShiyanlouCourseListSpider(Spider):
    ...
    def parse(self, response):
        ...
        next_url = 'https://www.shiyanlou.com/courses/?category=all&course_type=all&fee=all&tag=all&page=2'
        yield Request(url=next_url, callback=self.parse)  # 多米诺骨牌
